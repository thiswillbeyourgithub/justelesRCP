#!/bin/sh
# Container entrypoint for the justelesRCP static site. Three jobs before handing
# off to Caddy:
#
#   1. Stamp the container start time into STARTED_AT (epoch seconds) so the
#      optional DEV banner can show "restarted X ago" (src/dev-banner.js).
#
...
#      base URL: reachable, but returns HTML, so the injected <script> loads no
#      tracker and records zero events while looking configured. We refuse to
#      start rather than silently track nothing. Empty ANALYTICS_URL disables
...
#      docker/Caddyfile). The site root is mounted read-only, so we write to /gen
#      (a writable tmpfs).
set -eu
...
if [ -n "${ANALYTICS_URL:-}" ]; then
  ...
  # busybox wget: -S dumps response headers to stderr, body discarded. Non-zero
  ...
  # It resolved, but must be JavaScript, not the instance's HTML page. Take the
  # LAST Content-Type (final response after any redirects).
...
# so the umami tracker script and its beacon are allowed (script-src / connect-src).
...
# with the local-dev fallback in src/app-config.js.
